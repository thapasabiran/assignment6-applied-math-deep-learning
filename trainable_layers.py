...
def define_dense_model_with_hidden_layers(input_length, 
    ...
    model.add(layers.Dense(units=hidden_layers_sizes[0], input_dim=input_length, activation=activation_func_array[0]))

    # Create the hidden layers
    for i in range(1, len(hidden_layers_sizes)):
        model.add(layers.Dense(units=hidden_layers_sizes[i], activation=activation_func_array[i]))
